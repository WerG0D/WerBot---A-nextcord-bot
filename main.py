...
intents = nextcord.Intents().all() #Os intents (permissões) do bot, não recomendo deixar no all()
...
@tasks.loop(seconds=10) #Usando o modulo de tasks do discord.py pra criar a função de swap da atividade. Coloque um valor maior ou igual a 10 segundos, para não travar o bot.
...
async def on_application_command_error(ctx, error):
        if isinstance(error, commands.NotOwner):
            ...
            em = nextcord.Embed(title="Um erro ocorreu :(", description=f"```{error}```")
