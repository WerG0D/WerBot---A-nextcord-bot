...
from Misc.utilidades import Emojis, Icons, Wer
...
class HelpPageSource(menus.ListPageSource):
    ...
    async def format_page(self, menu: menus.ButtonMenuPages, entries: List[Tuple[str, str, str]]):
        ...
        embed.set_author(
            name=Wer.name, icon_url=Icons.questionmark)
        embed.description = (
...
class NewHelpCommand(commands.MinimalHelpCommand):
    ...
    async def command_not_found(self, string):
        ...
        await self.get_destination().send(embed=embed, view=DeletarMensagem(self.context))
    ...
    async def send_bot_help(self, mapping: dict):
        ...
        embed = nextcord.Embed(title=f"{Wer.name} Commands", colour=self.COLOUR)
        embed.set_author(
            name=Wer.name, icon_url=Icons.questionmark)
        embed.description = (
            f'Use `{prefix}{invoked_with} command`  for more info on a command.\n'
    ...
    async def send_cog_help(self, cog: commands.Cog):
        ...
        embed.set_author(name=Wer.name, icon_url=Icons.questionmark)
    ...
    async def send_command_help(self, command: commands.command):
        ...
        embed.set_footer(text=f"{Wer.name} ▶️ {command.qualified_name}", icon_url=Icons.questionmark) 
